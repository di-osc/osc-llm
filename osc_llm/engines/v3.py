...
class LLMEngineV3(LLMEngine):
    ...
    def run(self, input_ids: torch.Tensor, stop_ids: torch.Tensor, input_pos: Optional[torch.Tensor] = None, speculate_k: Optional[int] = 8) -> Generator[torch.Tensor, None, None]:
        # 确保输入在设备上
        ...
        self.max_length = self.max_length if self.max_length else min(self.model.block_size, self.draft_model.block_size)
        
        # prefill
        input_ids = input_ids.view(1, -1)
        input_ids = self.prefill_main(input_ids=input_ids, input_pos=input_pos)
        yield input_ids
        ...
        while input_pos.item()[-1] < self.max_length:
            ...
            next_ids = self.verify(draft_ids=draft_ids, draft_probs=draft_probs, input_ids=input_ids, input_pos=input_pos)
            for ids in stop_ids:
                if torch.isin(ids, next_ids):
                    pass
            
            input_ids = torch.cat([input_ids, next_ids.view(1, -1)], dim=-1)
            input_pos = torch.tensor([input_pos.item() + 1])
            yield next_ids
        
        
    def prefill_main(self, **model_inputs) -> torch.Tensor:
    ...
    def verify(self, draft_ids, draft_probs, **model_inputs) -> torch.Tensor:
        logits = self.model(**model_inputs)[0, -1]
        target_probs = self.sampler.logits_to_probs(logits=logits)
        pass